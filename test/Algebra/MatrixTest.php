<?php
/**
 *	TestUnit of CeusMedia\Math\Algebra\Matrix.
 ...
 *	@package		Tests.math.algebra
 *	@extends		Test_Case
 *	@uses			CeusMedia\Math\Algebra\Matrix
 *	@author			Christian Würker <christian.wuerker@ceusmedia.de>
 *	@since			17.06.2008
 *	@version		0.1
 */
class Test_Math_Algebra_MatrixTest extends Test_Case
{
	...
	{
		$this->matrix	= new CeusMedia\Math\Algebra\Matrix( 2, 3 );
	...
	{
		$matrix		= new CeusMedia\Math\Algebra\Matrix( 1, 3, 10 );
	...
	public function testConstructException1()
	{
		...
		new CeusMedia\Math\Algebra\Matrix( 0, 10 );
	}
	...
	{
		$this->setExpectedException( 'InvalidArgumentException' );
		new CeusMedia\Math\Algebra\Matrix( 10, 0 );
	}
	...
	{
		$assertion	= new CeusMedia\Math\Algebra\Vector( 1, 4 );
		$creation	= $this->matrix->getColumn( 0 );
		$this->assertEquals( $assertion, $creation );

		$assertion	= new CeusMedia\Math\Algebra\Vector( 2, 5 );
		$creation	= $this->matrix->getColumn( 1 );
		$this->assertEquals( $assertion, $creation );

		$assertion	= new CeusMedia\Math\Algebra\Vector( 3, 6 );
	...
	{
		$assertion	= new CeusMedia\Math\Algebra\Vector( 1, 2, 3 );
		$creation	= $this->matrix->getRow( 0 );
		$this->assertEquals( $assertion, $creation );

		$assertion	= new CeusMedia\Math\Algebra\Vector( 4, 5, 6 );
	...
	public function testSwapColumns()
	{
		...
		$assertion	= new CeusMedia\Math\Algebra\Vector( 2, 1, 3 );
		$creation	= $this->matrix->getRow( 0 );
		$this->assertEquals( $assertion, $creation );

		$assertion	= new CeusMedia\Math\Algebra\Vector( 5, 4, 6 );
		$creation	= $this->matrix->getRow( 1 );
		$this->assertEquals( $assertion, $creation );

		$this->matrix->swapColumns( 0, 2 );

		$assertion	= new CeusMedia\Math\Algebra\Vector( 3, 1, 2 );
		$creation	= $this->matrix->getRow( 0 );
		$this->assertEquals( $assertion, $creation );

		$assertion	= new CeusMedia\Math\Algebra\Vector( 6, 4, 5 );
		$creation	= $this->matrix->getRow( 1 );
		$this->assertEquals( $assertion, $creation );
	}
